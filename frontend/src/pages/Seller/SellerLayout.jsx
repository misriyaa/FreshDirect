...
const SellerLayout = () => {
    const { setisSeller, setUser, navigate } = useAppContext();
            ...
            <div className="flex items-center justify-between px-6 md:px-10 border-b border-[#e2e8f0] py-4 bg-white sticky top-0 z-50 shadow-sm">
                <Link to="/" className="flex items-center gap-2 no-underline group">
                    ...
                    <span className="text-md font-bold tracking-tight text-[#0f172a] hidden sm:block font-sans">
                        Nafeesathul Misriya <span className="text-[#1a8a50] font-mono text-xs ml-1 px-1.5 py-0.5 bg-[#1a8a50]/10 rounded font-normal">PORTAL</span>
                    </span>
                </Link>
                    ...
                    <div className="flex flex-col gap-1 px-2">
                        {sidebarLinks.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`transition-transform duration-200 group-hover:scale-105 ${isActive ? "text-[#1a8a50]" : "text-[#94a3b8]"}`}>
                                        {item.icon}
                                    </div>
                    ...
                    <Routes>
                        ...
                        <Route path="*" element={
                            <div className="bg-white p-8 rounded-xl border border-[#e2e8f0] text-center shadow-sm max-w-xl mx-auto my-10">
                                ...
                                <h3 className="text-xl font-bold m-0 mb-2 text-[#0f172a]">Welcome to Your Command Center</h3>
                                <p className="text-sm text-[#64748b] m-0 mb-6 leading-relaxed">
                                    Manage configurations across your local storefront seamlessly. Select an option from the sidebar to modify items.
                                </p>
                                <Link to="dashboard" className="inline-block bg-[#1a8a50] text-white font-semibold text-xs py-2.5 px-5 rounded-lg no-underline shadow-sm hover:bg-[#15703f] transition-all active:scale-95">
